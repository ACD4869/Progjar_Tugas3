...
while True:
    ...
    # Baca Username
    uname_cli = sock_cli.recv(65535).decode("utf-8")
    print(uname_cli, " joined")

    # Thread untuk membaca pesan
    thread_cli = threading.Thread(target=read_msg, args=(clients, sock_cli, addr_cli, uname_cli))
    thread_cli.start()

    # Simpan informasi klien ke dictionary
    clients[uname_cli] = (sock_cli, addr_cli, thread_cli)